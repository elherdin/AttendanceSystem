...
namespace AttendanceV2
{
    public partial class ParticipantForm : Form
    {
        ...
        {
            timer1.Start();
            label_hello.Text = "Hai, " + participantName + "!";
            ShowCurrentEvent();
        }
            ...
            try
            {
                ...
                using (MySqlConnection connection = DatabaseConnection.GetConnection())
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            label_currentEvent.Text = "Current Schedule: ";
                        ...
                        {
                            label_currentEvent.Text = "There is no ongoing schedule right now!";
        ...
        private void LoadDataToDataGridView()
        {
            using (MySqlConnection connection = DatabaseConnection.GetConnection())
            {
                try
                {
                    ...
                    string query = $"SELECT e.Name AS 'Class Name', e.StartDateTime AS 'Start Date', e.EndDateTime AS 'End Date', a.AttendStatus AS 'Kehadiran' FROM Events e INNER JOIN Attend a ON e.EventID = a.EventID INNER JOIN Participants p ON a.ParticipantID = p.ParticipantID WHERE p.UserID = {participantUserID}";
            ...
            try
            {
                using (MySqlConnection connection = DatabaseConnection.GetConnection())
                {
                    ...
                    if (eventID != null)
                    {
                        ...
                        if (attendCount > 0)
                        {
                            MessageBox.Show("You've already attended this schedule.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
                    ...
                    {
                        MessageBox.Show("There is no ongoing schedule to attend.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (MySqlException ex)
